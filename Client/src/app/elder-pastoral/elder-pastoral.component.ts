...
//     saveTeams(id?: any) {
//       console.log('draggedItem save components', id);
//       console.log('saveTeams components', this.teams);
//      let team: Team = this.getTeamToUpdate(this.teams || [], id);
//      console.log('team saveTeams components', team);
...
//       console.log('getTeamToUpdate components', teams, id);
//       console.log('getTeamToUpdate components id', teams[Number(id)]);
...
type DragItemType = 'member' | 'family';
...
export class ElderPastoralComponent implements OnInit, OnDestroy {
  public configForm!: FormGroup;
  ...
  public filteredElders: Members = [];
  public filteredFamilies: Families = [];

  // sources complètes
  public allElders: Members = [];
  ...
  ngOnInit(): void {
    ...
    combineLatest([
      ...
      .pipe(
        ...
        map(([teams, elders, families]) => {
          this.teams = teams || [];
          this.allElders = elders || [];
          this.allFamilies = families || [];

          this.configForm.patchValue(
            { maxTeams: this.teams.length },
            { emitEvent: false }
          );

          const usedMemberIds = new Set(
            this.teams.flatMap(team =>
              (team._members || []).map((m: any) => typeof m === 'string' ? m : m._id)
            )
          );

          const usedFamilyIds = new Set(
            this.teams.flatMap(team =>
              (team._families || []).map((f: any) => typeof f === 'string' ? f : f._id)
            )
          );

          return {
            filteredElders: this.allElders.filter(e => !usedMemberIds.has(e._id)),
            filteredFamilies: this.allFamilies.filter(f => !usedFamilyIds.has(f._id))
          };
        }),
        takeUntil(this._unsubscribeAll)
      )
  ...
  // =========================================================
  // HELPERS IDS CONTAINERS
  // =========================================================

  private isStockMembers(id: string): boolean {
    return id === 'pretriseMembers';
  }

  private isStockFamilies(id: string): boolean {
    return id === 'familyMembers';
  }

  private isTeamMembersContainer(id: string): boolean {
    return id.startsWith('members-');
  }

  private isTeamFamiliesContainer(id: string): boolean {
    return id.startsWith('families-');
  }

  private isTeamContainer(id: string): boolean {
    return this.isTeamMembersContainer(id) || this.isTeamFamiliesContainer(id);
  }

  private getTeamIndexFromContainerId(id: string): number {
    const parts = id.split('-');
    if (parts.length < 2) return NaN;
    return Number(parts[1]);
  }

  getTeamToUpdate(teams: Teams, id: string): Team | null {
    const index = this.getTeamIndexFromContainerId(id);
  ...
  getDragFamilyData(family: Family, type: string): Family & { type: DragItemType } | null {
    if (type === 'family') {
      return { ...family, type: 'family' };
    }
  ...
  getDragMemberData(member: Member, type: string): Member & { type: DragItemType } | null {
    if (type === 'member') {
      return { ...member, type: 'member' };
    }
  ...
  dropMember(event: CdkDragDrop<any[]>): void {
    const draggedItem = event.item.data;

    if (draggedItem?.type !== 'member') {
      this._snackBar.open('Cette famille n\'est pas un Frère', 'Fermer', {
        duration: 5000,
        horizontalPosition: 'right',
        verticalPosition: 'top'
      });
    ...
    // stock -> team
    if (this.isStockMembers(previousId) && this.isTeamMembersContainer(currentId)) {
      this.saveTeamByContainerId(currentId);
      return;
    }

    // team -> stock
    if (this.isTeamMembersContainer(previousId) && this.isStockMembers(currentId)) {
      this.saveTeamByContainerId(previousId);
      return;
    }

    // team -> team
    if (this.isTeamMembersContainer(previousId) && this.isTeamMembersContainer(currentId)) {
      this.saveTwoTeams(previousId, currentId);
      return;
    }
  }
  ...
  dropFamily(event: CdkDragDrop<any[]>): void {
    const draggedItem = event.item.data;
    ...
    // stock -> team
    if (this.isStockFamilies(previousId) && this.isTeamFamiliesContainer(currentId)) {
      this.saveTeamByContainerId(currentId);
      return;
    }

    // team -> stock
    if (this.isTeamFamiliesContainer(previousId) && this.isStockFamilies(currentId)) {
      this.saveTeamByContainerId(previousId);
      return;
    }

    // team -> team
    if (this.isTeamFamiliesContainer(previousId) && this.isTeamFamiliesContainer(currentId)) {
      this.saveTwoTeams(previousId, currentId);
      return;
    }
  }
  ...
  saveTeamByContainerId(id: string): void {
    const team = this.getTeamToUpdate(this.teams, id);

    if (!team) {
      console.warn('Aucune équipe trouvée pour le container :', id);
      return;
    }

    this._teamsService.updateTeams(this.region, team).subscribe({
      next: () => {
        this._snackBar.open('Équipe mise à jour !', 'Fermer', {
          duration: 3000
        });
      },
      error: (error) => {
        console.error(error);
        this._snackBar.open('Erreur lors de la mise à jour de l\'équipe', 'Fermer', {
          duration: 5000
        });
      }
    });
  ...
  saveTwoTeams(previousId: string, currentId: string): void {
    const previousTeam = this.getTeamToUpdate(this.teams, previousId);
    const currentTeam = this.getTeamToUpdate(this.teams, currentId);
    ...
    if (previousTeam) {
      this._teamsService.updateTeams(this.region, previousTeam).subscribe({
        error: (error) => console.error('Erreur update previousTeam', error)
      });
    }

    if (currentTeam) {
      this._teamsService.updateTeams(this.region, currentTeam).subscribe({
        next: () => {
          this._snackBar.open('Équipes mises à jour !', 'Fermer', {
            duration: 3000
          });
        },
        error: (error) => {
          console.error('Erreur update currentTeam', error);
          this._snackBar.open('Erreur lors de la mise à jour des équipes', 'Fermer', {
            duration: 5000
          });
        }
      });
  ...
  updateAvailableLists(): void {
    const usedMemberIds = new Set(
      this.teams.flatMap((team: Team) =>
        (team._members || []).map((m: any) => m._id)
      )
    );

    const usedFamilyIds = new Set(
      this.teams.flatMap((team: Team) =>
        (team._families || []).map((f: any) => f._id)
      )
    );

    this.filteredElders = this.allElders.filter((m: Member) => !usedMemberIds.has(m._id));
    this.filteredFamilies = this.allFamilies.filter((f: Family) => !usedFamilyIds.has(f._id));
  }
  ...
  updateTeams(event: Event): void {
    ...
    this._teamsService.createConfigTeam(this.region, config).subscribe({
      next: () => {
        this._snackBar.open(`Le nombre d'équipe a été modifié à ${config.maxTeams} avec succès`, 'Fermer', {
          duration: 5000,
          horizontalPosition: 'right',
          verticalPosition: 'top'
        });
  ...
  onlyMembers = (drag: CdkDrag, drop: CdkDropList): boolean => {
    return drag.data?.type === 'member';
  };

  onlyFamilies = (drag: CdkDrag, drop: CdkDropList): boolean => {
    return drag.data?.type === 'family';
  };
  ...
  getMemberName(id: string): string {
    let member = this.allElders.find(m => m._id === id);

    this.teams.forEach(team => {
      const teamMember = team?._members?.find((m: any) => m._id === id);
      if (teamMember) {
        member = teamMember;
      }
    });
  ...
  getFamilyName(id: string): string {
    let family = this.allFamilies.find(f => f._id === id);

    this.teams.forEach(team => {
      const teamFamily = team?._families?.find((f: any) => f._id === id);
      if (teamFamily) {
        family = teamFamily;
      }
    });
